    ...
    def pocket_pla_1(self, X_train, Y_train, X_test, Y_test):
        ...
        for iters in range(Iteration):
            ...
            for u in range(Update):
                for i in range(N):
                    index = randomIndex[i]
                    y_hat = np.sign(np.dot(X_train[index, :], W))
                    err_cnt = self.calError(X_train, Y_train, W)
                    if y_hat * Y_train[index] <= 0:
                        fault += 1
                        W_redress = W + Y_train[index] * X_train[index, :]
                        if self.calError(X_train, Y_train, W_redress) < err_cnt:
                            W = W_redress
            Faults.append(fault / Update)
            error = self.calError(X_test, Y_test, W)
#             print("iter[{}]---error:{}".format(iters, error))
            Errors.append(error / X_test.shape[0])


#         plt.hist(Faults,
#                  bins=40,
#                  color='cyan')
#         plt.xlim(min(Faults), max(Faults))
#         plt.xlabel('# fault')
#         plt.ylabel('Frequency')
#         plt.title('histogram in pocket_pla_1')
#         plt.show()

        plt.hist(Errors,
                 bins=40,
                 color='steelblue')
        plt.xlim(min(Errors), max(Errors))
        plt.xlabel('# error')
        plt.ylabel('Frequency')
        plt.title('histogram in pocket_pla_1')
        plt.show()
...
#         plt.show()
        ...
        plt.xlabel('# error')
        plt.ylabel('Frequency')
        plt.title('histogram in pocket_pla_2')
        plt.show()
        ### END YOUR CODE
...
#         plt.show()
        ...
        plt.xlabel('# error')
        plt.ylabel('Frequency')
        plt.title('histogram in pocket_pla_3')
        plt.show()
        ### END YOUR CODE
